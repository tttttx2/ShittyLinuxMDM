    ...
    if ("table" in data){
      if (data['table'] == false){
       ...
       document.getElementById("results").innerHTML = tablecode
        ...
        data['table'].forEach((element) => tablecode+='<tr><td>'+Object.values(element).join('</td><td>')+'</td><td>'+(('readonly' in data)?(''):('<a onclick="start_feature_read(\''+plugin+'\', \''+feature+'\', \''+element['id']+'\')">Edit</a>'))+'</td><td>'+(('readonly' in data)?(''):('<a onclick="start_feature_delete(\''+plugin+'\', \''+feature+'\', \''+element['id']+'\')">Delete</a>'))+'</td></tr>');
        // tablecode+='<tr><td>NEW</td>';
        // Object.keys(data['table'][0]).forEach((element) => tablecode+='<td></td>');
        // tablecode += '<td></td></tr>'
        tablecode += '</tbody></table>'
        if ("readonly" in data == false){
